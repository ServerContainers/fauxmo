...
if [ -z ${POSTFIX_VERSION+x} ] || [ -z ${DOVECOT_VERSION+x} ] || [ -z ${ALPINE_VERSION+x} ]; then
  ...
  export FAUXMO_VERSION=$(docker run --rm -ti "$IMG" pip list | grep fauxmo | tr ' ' '\n' | grep '\.')
...
if echo "$@" | grep -v "force" 2>/dev/null >/dev/null; then
  ...
  echo "$@" | grep -v "release" && docker pull "$IMG:a$ALPINE_VERSION-f$FAUXMO_VERSION" 2>/dev/null >/dev/null && echo "image already build" && exit 1
fi

docker buildx build -q --pull --no-cache --platform "$PLATFORM" -t "$IMG:a$ALPINE_VERSION-f$FAUXMO_VERSION" --push .
